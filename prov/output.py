...
class GraphTransformer():
    ...
    def create_dgl_graph(self, G):
        ...
        DG = dgl.graph(G.get_graph_dictionary())
        print("Graph constructed.")
        return DG
...
class GraphSerializer():
    ...
    def graph_to_json(self, G):
        ...
        out2 = self.output_path + "/jsons/graph{}.json".format(str(self.id))
        with open(f'{out2}', 'w') as f:
            json.dump(output_dict, f)

        print("Graph outputted to " + out + ".")

    def graph_info_to_file(self, G):
        out = self.output_path + "/stats".format(str(self.id))
        if not os.path.exists(out):
            os.makedirs(out)
        out += "/stats{}.csv".format(str(self.id), str(self.id))
        with open(f'{out}', 'w') as f:
